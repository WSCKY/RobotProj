...
{
  uint32_t _init_ptr = _ec20_ptr_out;
  while(len --) {
    if(*pSrc != ec20if_cache[_init_ptr]) {
      return status_error;
    }
    pSrc ++;
    _init_ptr ++;
    if(_init_ptr == EC20IF_CACHE_DEPTH) _init_ptr = 0;
  }
  return status_ok;
}
...
status_t ec20_check_ack(uint8_t *p, uint32_t timeout)
{
  ...
  while(_tick_end > EC20_Ticks()) {
    while(_ec20_data_available >= size) {
      if(cmp_ack(p, size) == status_ok) {
        _ec20_ptr_out += _ec20_data_available;
        if(_ec20_ptr_out >= EC20IF_CACHE_DEPTH)
          _ec20_ptr_out -= EC20IF_CACHE_DEPTH;
        _ec20_data_available = 0;
        return status_ok;
      } else {
        _ec20_ptr_out ++;
