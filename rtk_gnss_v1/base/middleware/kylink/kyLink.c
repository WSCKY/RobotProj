...
status_t kyLinkInit(KYLINK_CORE_HANDLE *pHandle)
{
	pHandle->port_tx = kyNULL;
	pHandle->decoder.rx_counter = 0;
	pHandle->decoder.update_flag = false;
  pHandle->decoder.decode_callback = kyNULL;
	pHandle->decoder._decode_state = DECODE_STATE_UNSYNCED;
  return status_ok;
}

void kyLinkConfigCbFunc(KYLINK_CORE_HANDLE *pHandle, kyLinkDecodeCallback pCb)
{
  pHandle->decoder.decode_callback = pCb;
}

void kyLinkConfigTxFunc(KYLINK_CORE_HANDLE *pHandle, kyLinkPortTxBytesFunc pTx)
{
  pHandle->port_tx = pTx;
}

bool_t kyLinkCheckUpdate(KYLINK_CORE_HANDLE *pHandle)
{
	if(pHandle->decoder.update_flag == true) {
		pHandle->decoder.update_flag = false;
		return true;
	}
	return false;
...
status_t SendTxPacket(KYLINK_CORE_HANDLE *pHandle, kyLinkPackageDef* pPacket)
{
  ...
  *(uint16_t *)&(pPacket->FormatData.PacketData.RawData[pPacket->FormatData.length]) = pPacket->FormatData.crc16;
  if(pHandle->port_tx != kyNULL)
	  return pHandle->port_tx(pPacket->RawData, pPacket->FormatData.length + 8);
  return status_error;
}
  ...
  switch(pHandle->decoder._decode_state) {
	...
	case DECODE_STATE_GOT_CRC_L:
      ...
      if(pHandle->decoder.decode_callback != kyNULL) {
        ...
        pHandle->decoder.update_flag = true;
      }
	  } else {}
      pHandle->decoder._decode_state = DECODE_STATE_UNSYNCED;
	break;
